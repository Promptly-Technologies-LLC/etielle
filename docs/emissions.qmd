...
``` {python}
from etielle import MappingSpec, TraversalSpec, fields, InstanceEmit, FieldSpec, PydanticBuilder, AppendPolicy
from etielle.transforms import get, literal
from etielle.executor import run_mapping
from pydantic import BaseModel
...
emit1 = InstanceEmit[User](
    ...
    fields=[
        FieldSpec(selector=fields(User).id, transform=get("id")),
        FieldSpec(selector=fields(User).tags, transform=literal("verified")),
    ],
    builder=PydanticBuilder(User),
...
emit2 = InstanceEmit[User](
    ...
    join_keys=[get("id")],
    fields=[
        FieldSpec(selector=fields(User).tags, transform=literal("premium")),
    ],
    builder=PydanticBuilder(User),
    policies={"tags": AppendPolicy()},
)
...
Use `fields()` for type-checked field references:
...
# fields(User).emial  # Error!
```

When to use field selectors vs strings:

| Use `fields()` when... | Use strings when... |
|------------------------|---------------------|
| Working with typed models | Working with plain dicts |
| Using a type checker (pyright, ty, mypy) | Prototyping quickly |
