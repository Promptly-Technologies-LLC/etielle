...
4. **Nesting requires `.goto()` between `.each()` calls**: You cannot use `.each().each()` directly—there must be a `.goto("field")` between iterations. For example, `.each().goto("choices").each()` works, but `.each().each()` on a dict-of-lists does not.
