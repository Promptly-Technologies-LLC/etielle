...
``` {python}
from etielle import MappingSpec, TraversalSpec, fields, InstanceEmit, FieldSpec, PydanticBuilder
...
posts_emit = InstanceEmit[Post](
    ...
    join_keys=[get("id")],
    fields=[
        FieldSpec(selector=fields(Post).id, transform=get("id")),
        FieldSpec(selector=fields(Post).title, transform=get("title")),
    ],
...
For a complete SQLAlchemy integration example with automatic flushing, see [Database upserts](loading-data-into-a-database.qmd).

### See also

- [Database upserts](loading-data-into-a-database.qmd) - Full integration with SQLAlchemy/SQLModel ORM
- [Emissions](emissions.qmd) - Creating the instances that relationships link
